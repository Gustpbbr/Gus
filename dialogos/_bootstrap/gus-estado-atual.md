...
- [episodico/gus] Antes de implementar a Fase 1 do NeuroGus, é necessário revisar toda a documentação do projeto para ter uma visão completa do que foi feito até agora. _(via claude-code)_
- [fato/projetos] Tipo de fragmento no NeuroGus: 'cada fragmento é uma esfera', conexão por afinidade semântica. _(via claude-code)_
- [projeto/projetos] Cada fragmento no NeuroGus é uma esfera, com conexão por afinidade semântica usando cosine top-K. _(via claude-code)_
- [decisao/projetos] Três decisões no projeto NeuroGus precisam ser confirmadas antes do desenvolvimento começar. _(via claude-code)_
- [decisao/projetos] A proposta padrão para o sistema de afinidade no NeuroGus é K=3 e threshold=0.6 (cosine). _(via claude-code)_
- [fato/projetos] A última versão do neurogus.html conta com interatividade completa, permitindo a visualização de nós e conexões. _(via claude-code)_
- [fato/projetos] O fragmento de código para criar hub/events.py (fila SSE) está nas etapas iniciais da implementação do NeuroGus. _(via claude-code)_
- [fato/projetos] Status atual do NeuroGus: backend (Hub, curador, embeddings, FastAPI, Bearer): pronto, código novo do NeuroGus: 0 linhas escritas. _(via claude-code)_
...
- **Total no brain `gustavo`**: 5272 fragmentos
